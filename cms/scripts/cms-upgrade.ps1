...
function Create-DxaFrameworkSubtree()
{
    ...
    $templatesFolderWebDavUrl = "$rootFolderWebDavUrl/$frameworkFolder/Developer/Templates"
    $dxaUpgradeFolderWebDavUrl = "$templatesFolderWebDavUrl/DXA%2EUpgrade"
    ...
    $upgrade = $true
    Write-Host "checking $dxaUpgradeFolderWebDavUrl"
    if ($coreServiceClient.IsExistingObject("$dxaUpgradeFolderWebDavUrl"))
    {
        $upgrade = $false
        Write-Host "Folder '$dxaUpgradeFolderWebDavUrl' already exists; it seems that your CMS already has DXA 2.0 structure."
        if (!$force)
        {
            Write-Host "Specify the -force flag if you want the run the upgrade anyways."
        }
    ...
    if($upgrade -or $force)
    {   
        $fwDeveloperTemplatesUpgradeFolderData = Create-Folder "DXA.Upgrade" $templatesFolderWebDavUrl
    
        Move-Item "$templatesFolderWebDavUrl/DD4T%2ETemplates" $fwDeveloperTemplatesUpgradeFolderData
        Move-Item "$templatesFolderWebDavUrl/Legacy" $fwDeveloperTemplatesUpgradeFolderData
        Move-Item "$templatesFolderWebDavUrl/Sdl%2EWeb%2ETridion%2ETemplates" $fwDeveloperTemplatesUpgradeFolderData        
    }
}
