...
# TODO: Make it possible to pass $module as Array of strings with title $modules
Param (
...
function Get-CoreServiceClient {
	...
	switch($type)
	{
		"Service" {
			...
			$endpoint = New-Object System.ServiceModel.EndpointAddress ($cmsUrl + "/webservices/CoreService201701.svc/wsHttp")
			$client = New-Object Tridion.ContentManager.CoreService.Client.SessionAwareCoreServiceClient $binding, $endpoint

            $credentials = New-Object System.Net.NetworkCredential($cmsUserName, $cmsUserPassword)
