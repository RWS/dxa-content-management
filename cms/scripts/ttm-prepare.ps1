...
function Get-DxaWebApplications($cdEnvironment)
{
    ...
    else
    {
        do
        {
            ...
            Write-Host ("Add-TtmWebsite -CdEnvironmentId $($cdEnvironment.Id) -BaseUrls $($baseUrls) -ErrorVariable cmdletError")
            $website = Add-TtmWebsite -Id $webSite -CdEnvironmentId $cdEnvironment.Id -BaseUrls $baseUrls -ErrorVariable cmdletError
